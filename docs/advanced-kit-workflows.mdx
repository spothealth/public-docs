...
Alternatively, if you use the [patient registration token](reference/generate-registration-token) endpoint to control
the registration flow, you can specify which kit type you'd like to use as part of the patient token. This can be used
in conjunction with retail kits to build custom registration flows for many tests that only require you to stock a
single kit type.
