...
you can add and remove SKUs, and map them to Spot kit types. Use the inline creation form to set up your SKU mappings.
2. Once you've created your ecommerce integration, copy the webhook URL provided in the main Ecommerce integrations
table. This is the URL that Shopify will use to notify Spot when an order is placed.
3. In your Shopify admin panel, select "Settings" from the bottom left corner, then "Notifications" from the submenu
that appears.
4. Scroll down to the "Webhooks" section and click "Create webhook".
5. Paste the webhook URL you copied from the Spot management portal into the "URL" field.
6. In the "Event" dropdown, select "Order payment".
7. Click save.
8. Make sure your Shopify checkout page requires an email address. Set the required customer contact information to
"Email address only" in your Shopify admin panel by selecting "Settings" from the bottom left corner, then "Checkout"
from the submenu that appears.
9. Place a test order manually and let us know so we can confirm the integration has been installed correctly.
